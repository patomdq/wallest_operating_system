...
export default function CalculadoraRentabilidad() {
  ...
  const handleEditarProyecto = (proyecto: ProyectoRentabilidad) => {
    ...
    setFormDetalle({
      ...
      valor_referencia: proyecto.valor_referencia?.toString() || '',
      vendedor_no_residente: proyecto.vendedor_no_residente || false,
    });
    ...
    setConceptos([
      ...
      { id: 4, nombre: 'Impuestos de compra ITP', estimado: proyecto.impuestos_compra_estimado?.toString() || '', real: proyecto.impuestos_compra_real?.toString() || '' },
      { id: 5, nombre: 'Retenciones extranjeros', estimado: proyecto.retenciones_extranjeros_estimado?.toString() || '', real: proyecto.retenciones_extranjeros_real?.toString() || '' },
      { id: 6, nombre: 'Liquidación complementaria', estimado: proyecto.liquidacion_complementaria_estimado?.toString() || '', real: proyecto.liquidacion_complementaria_real?.toString() || '' },
    ]);

    setGastosSeleccionados(proyecto.gastos_checklist || []);
  };
                  ...
                  {proyectos.map((proyecto) => {
                    ...
                    return (
                      <tr key={proyecto.id} className="border-b border-wos-border/50 hover:bg-wos-bg/50 transition-smooth">
                        ...
                        <td className="py-3 px-4 text-wos-text-muted">{proyecto.comunidad || '-'}</td>
      ...
      <div className="bg-wos-card border border-wos-border rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div>
            ...
            <select
              value={formDetalle.comunidad}
              onChange={(e) => setFormDetalle({ ...formDetalle, comunidad: e.target.value })}
              className="w-full bg-wos-bg border border-wos-border rounded-lg px-4 py-2 text-wos-accent focus:outline-none focus:ring-2 focus:ring-wos-primary"
            >
              <option value="">Seleccionar...</option>
              {COMUNIDADES.map(com => (
