...
import { supabase, Finanza } from '@/lib/supabase';
...
export default function Finanzas() {
  ...
  const loadFinanzas = async () => {
    const { data } = await supabase.from('finanzas').select('*').order('fecha', { ascending: false });
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-wos-card border border-wos-border rounded-lg p-6">
          ...
          <p className="text-3xl font-bold text-green-500">€{totalIngresos.toLocaleString()}</p>
        </div>
        <div className="bg-wos-card border border-wos-border rounded-lg p-6">
        ...
        <div className="bg-wos-card border border-wos-border rounded-lg p-6">
          ...
          <p className={`text-3xl font-bold ${totalIngresos - totalGastos >= 0 ? 'text-green-500' : 'text-red-500'}`}>
      ...
      <div className="bg-wos-card border border-wos-border rounded-lg overflow-hidden">
        <table className="w-full">
          <thead className="bg-wos-bg border-b border-wos-border">
            <tr>
              <th className="text-left px-6 py-4 text-sm font-semibold text-wos-text-muted">Fecha</th>
              <th className="text-left px-6 py-4 text-sm font-semibold text-wos-text-muted">Concepto</th>
              <th className="text-left px-6 py-4 text-sm font-semibold text-wos-text-muted">Tipo</th>
              <th className="text-left px-6 py-4 text-sm font-semibold text-wos-text-muted">Monto</th>
              <th className="text-right px-6 py-4 text-sm font-semibold text-wos-text-muted">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {finanzas.map((f) => (
              <tr key={f.id} className="border-b border-wos-border hover:bg-wos-bg">
                <td className="px-6 py-4 text-wos-text-muted">{f.fecha}</td>
                <td className="px-6 py-4 text-wos-text">{f.concepto}</td>
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    f.tipo === 'ingreso' ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'
                  }`}>
                    {f.tipo}
                  </span>
                </td>
                <td className={`px-6 py-4 font-semibold ${f.tipo === 'ingreso' ? 'text-green-500' : 'text-red-500'}`}>
